...
- **`EmbeddedField`** carries the full property set: an `EmbeddedArtifactKey`, a typed reference to the embedded `Field`, and optional `ValueRequirement`, `Cardinality`, `Visibility`, family-typed `defaultValue`, `PromptOverride`, and `Property` (a semantic property IRI for the embedding site).
- **`EmbeddedTemplate`** carries the embedding key, the embedded template's identifier, and optional `ValueRequirement`, `Cardinality`, `Visibility`, `PromptOverride`, and `Property`. It carries no `defaultValue` (templates do not have value-typed defaults).
...
classDiagram
  ...
  IntegerField --> IntegerFieldSpec : carries
  DecimalField --> DecimalFieldSpec : carries
  FloatField --> FloatFieldSpec : carries
  DoubleField --> DoubleFieldSpec : carries
