...
- Repeated components (`X*` / `X+`) are `list[T]` and are **never `None` and never optional**: an absent or empty repetition is the empty list `[]`, not `None`. Type them `list[T]` (not `list[T] | None`) with `Field(default_factory=list)` so a missing input becomes `[]`. Callers iterate without a `None` check. (Only scalar optionals use `T | None`.) For `X+` (one-or-more), enforce non-empty with `Field(min_length=1)` or a `model_validator`.
- Empty `X*` slots round-trip as absent: omit an empty repeated slot on encode (e.g. `alt_prompts`, `annotations`, `examples`, `members`) and treat an omitted such slot as `[]` on decode. Because empty lists must be omitted (not emitted as `[]`), do not rely on `exclude_none` alone for lists; omit empty collections explicitly.
