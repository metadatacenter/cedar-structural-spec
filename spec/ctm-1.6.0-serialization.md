...
12. **`Field.RecommendedKey`** — The advisory `RecommendedKey` slot on every concrete `Field` is dropped in the CTM 1.6.0 encoding: there is no legacy slot to carry a field-owner's suggested embedding key, and the legacy model does not surface authoring-time hints of this kind. Because `RecommendedKey` is non-normative (it does not constrain instance data or any other operational behaviour), dropping it on encode is safe; round-tripping a CTM 1.6.0 document back to the Structural Model omits the slot.

13. **`Field.RecommendedProperty`** — The advisory `RecommendedProperty` slot on every concrete `Field` is dropped in the CTM 1.6.0 encoding: there is no legacy slot to carry a field-owner's suggested embedding property IRI, and the legacy model does not surface authoring-time hints of this kind. Because `RecommendedProperty` is non-normative (it does not constrain `EmbeddedField.Property` or any other operational behaviour), dropping it on encode is safe; round-tripping a CTM 1.6.0 document back to the Structural Model omits the slot. The authoritative predicate for an embedding's RDF projection in CTM 1.6.0 output remains the `EmbeddedField.Property` carried by each embedding site, exactly as in the Structural Model.

14. **`Field.AlternativePrompt*`** — The curated set of alternative question wordings carried by each concrete `Field` (`altPrompts` on the wire) is dropped in the CTM 1.6.0 encoding: the legacy model has a single rendered prompt per field and no carrier for a field-owner-curated set of keyed alternative wordings. Dropping the set on encode does not affect the rendered output, because CTM 1.6.0 always renders the field's single preferred prompt (`Field.Prompt`). Round-tripping a CTM 1.6.0 document back to the Structural Model omits the slot — the curated alternatives are not recoverable, so an embedding's keyed selection of an alternative (see gap 15) cannot be reconstructed either.

15. **`EmbeddedField.PromptKey`** — The embedding-side `PromptKey` slot (`promptKey` on the wire), which selects one of the referenced field's `AlternativePrompt` entries, is dropped in the CTM 1.6.0 encoding: there is no legacy slot for a keyed selection into a curated wording set, and the alternatives themselves are not encoded (gap 14). On encode, an embedding that selected an alternative wording is rendered using the field's preferred `Prompt` — the same fallback as an embedding that selected nothing. This is lossy: the distinction between "selected the acronym wording" and "used the default" is not preserved. Round-tripping back to the Structural Model omits the slot. (The free-form `EmbeddedField.PromptOverride`, by contrast, maps to the legacy per-embedding prompt override where one exists; `PromptKey` has no such legacy analogue because the legacy model has no curated-alternatives concept.)

16. **`Section` grouping is flattened.** CTM 1.6.0 represents a template's member arrangement only as a flat `_ui.order` array of embedded-artifact keys; it has no construct for grouping members under a heading. On encode, the Structural Model's member tree is therefore flattened: the encoder walks the tree in document order, recursing into `Section` bodies, and emits every embedded artifact's key into `_ui.order` exactly as for an un-sectioned template. The section boundaries, each `Section.Label` and `Section.Description`, and the `Collapsibility` declaration are all dropped — there is no legacy slot for any of them. Because instance values are keyed only by `EmbeddedArtifactKey` (which the flattening preserves), no instance data is lost; only the semantic grouping is. Round-tripping a CTM 1.6.0 document back to the Structural Model yields a flat, section-free template.

17. **`EmbeddedField.Editability`** — The read-only/editable distinction is dropped in the CTM 1.6.0 encoding. The embedding `_ui` block emitted by `encode_embedding_ui` carries only a `hidden` flag (derived from `Visibility`); the legacy form has no key modelled here for read-only intent. An `Editability` of `"readOnly"` is therefore not represented on encode, and the field is emitted as an ordinary editable field (subject to its `Visibility`). This is lossy: a read-only field and an editable one with the same definition encode identically. Round-tripping back to the Structural Model omits the slot (the field defaults to `"editable"`). The `readOnly + required + no-default` validation rule is a Structural Model concern and has no bearing on the CTM 1.6.0 output, which does not enforce it.
